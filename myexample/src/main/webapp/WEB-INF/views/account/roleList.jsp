...
<body>
	...
	<div class="row" style="margin-top:60px">
		<div class="span4 offset7">
			<form class="form-search" action="${ctx }/role/List" method="post">
			
				<label>角色名：</label> 
				<form class="navbar-search pull-left">
				  	<input type="text" class="search-query input-medium" placeholder="Search" name="search_LIKE_roleName"  value="${param.search_LIKE_roleName}"/>
				</form>
		    </form>
	    </div>
	</div> 
	<table id="contentTable" class="table table-striped table-bordered table-condensed topForm">
		...
		<c:forEach items="${objects.content}" var="entity">
			<tr>
				...
				<td><a href="${ctx}/role/delete/${entity.id}">删除</a></td>
			</tr>
		</c:forEach>
		</tbody>
